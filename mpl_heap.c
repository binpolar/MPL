...
bool mpl_heap_push(mpl_heap_entry_t *node)  // Changed to pointer
{
    if (!node) return false;
    return cplus_heap_push(&heap_desc, (cplus_heap_entry_t*)node);
}

bool mpl_heap_pop(mpl_heap_entry_t *out_element)  // Returns bool for success/failure
{
    if (!out_element) return false;
